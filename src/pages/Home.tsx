...
import { ScrollBoundaryProvider } from '@/components/chat/ScrollBoundaryContext';
...
export default function Home() {
  ...
  /** 피드백 팝오버가 넘어가지 않을 기준 영역 — footer(sticky) 뺀 메시지 영역만.
      렌더 중에 ref.current를 못 읽어서 콜백 ref로 state에 담는다. */
  const [scrollBoundaryEl, setScrollBoundaryEl] = useState<HTMLDivElement | null>(null);
    ...
    <div className="bg-background-canvas flex h-screen">
      ...
      <div className="bg-background-canvas-fade-solid flex min-h-0 min-w-0 flex-1 flex-col">
        ...
        <main className="riido-scrollbar relative min-h-0 flex-1 [scrollbar-gutter:stable] overflow-y-auto">
          <ScrollBoundaryProvider value={scrollBoundaryEl}>
            ...
            <div className="flex min-h-full flex-col">
              ...
              {/* ScrollBoundaryProvider: 피드백 팝오버 collisionBoundary용. footer가 main 전체 높이 중
                sticky로 얹혀 있어서 main 자체를 기준(clipping-ancestors 기본값)으로 두면 팝오버가
                footer/트리거 줄과 겹치는 걸로 계산 안 해서 그 위에 겹쳐 뜬다. */}
              <div ref={setScrollBoundaryEl} className="flex flex-1 flex-col">
                <div
                  className={cn(
